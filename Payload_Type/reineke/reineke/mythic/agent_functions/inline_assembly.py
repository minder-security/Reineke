...
if platform.system() == 'Windows':
    INTEROP_ASSEMBLY_PATH = "C:\\Mythic\\Reineke\\srv\\ReinekeInterop.dll"
else:
    INTEROP_ASSEMBLY_PATH = "/srv/ReinekeInterop.dll"
INTEROP_FILE_ID = ""
...
class InlineAssemblyCommand(CommandBase):
    ...
    async def build_interop(self):
        ...
        outputPath = "{}/ReinekeInterop/bin/Release/ReinekeInterop.dll".format(agent_build_path.name)
        copy_tree(str(self.agent_code_path), agent_build_path.name)
        shell_cmd = "dotnet build -c release -p:DebugType=None -p:DebugSymbols=false -p:Platform=x64 {}/ReinekeInterop/ReinekeInterop.csproj -o {}/ReinekeInterop/bin/Release/".format(
            agent_build_path.name, agent_build_path.name)
        proc = await asyncio.create_subprocess_shell(shell_cmd, stdout=asyncio.subprocess.PIPE,
                                                     stderr=asyncio.subprocess.PIPE, cwd=agent_build_path.name)
        stdout, stderr = await proc.communicate()
        if not path.exists(outputPath):
            raise Exception("Failed to build ReinekeInterop.dll:\n{}".format(stderr.decode() + "\n" + stdout.decode()))
