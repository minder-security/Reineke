...
NOTE: v2.3.2+ has a different bof loader than 2.3.1 and are incompatible since their arguments are different
    ...
    agent_path = pathlib.Path(".") / "reineke" / "mythic"
    agent_code_path = pathlib.Path(".") / "reineke" / "agent_code"
    ...
    async def build(self) -> BuildResponse:
        ...
        success_message = f"reineke {self.uuid} Successfully Built"
        stdout_err = ""
        ...
        for c2 in self.c2info:
            ...
            for key, val in c2.get_parameters_dict().items():
                ...
                if isinstance(val, dict) and 'enc_key' in val:
                    if val["value"] == "none":
                        resp.set_status(BuildStatus.Error)
                        resp.set_build_message("reineke does not support plaintext encryption")
                        return resp
        ...
        try:
            ...
            output_path = f"{agent_build_path.name}/{buildPath}/Reineke.exe"
            if self.get_parameter('debug'):
            ...
            if os.path.exists(output_path):
                ...
                targetInteropPath = "/srv/ReinekeInterop.dll"
                ...
                shutil.move(f"{agent_build_path.name}/{buildPath}/ReinekeInterop.dll", targetInteropPath)
